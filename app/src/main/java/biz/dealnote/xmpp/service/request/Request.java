...
public final class Request implements Parcelable {
    ...
    public Request putParcelableArrayList(String name, ArrayList<? extends Parcelable> value) {
        removeFromRequestData(name);
        mParamList.add(name);
        mTypeList.add(TYPE_PARCELABLE_ARRAY_LIST);
    ...
    public <T extends Parcelable> ArrayList<T> getParcelableArrayList(String name) {
            ...
            for (int i = 0, length = mParamList.size(); i < length; i++) {
                ...
                switch (mTypeList.get(i)) {
                    ...
                    case TYPE_DOUBLE:
                        ...
                        break;
                    case TYPE_STRING:
                        if (!Utils.safeEquals(mBundle.getString(param),
                                oParams.mBundle.getString(param))) {
                            return false;
                        }
                        break;
                    case TYPE_CHARSEQUENCE:
                        if (!Utils.safeEquals(mBundle.getCharSequence(param),
                                oParams.mBundle.getCharSequence(param))) {
                            return false;
                        }
                        break;
                    case TYPE_PARCELABLE:
                        if (!Utils.safeEquals(mBundle.getParcelable(param),
                                oParams.mBundle.getParcelable(param))) {
                            return false;
                        }
                        break;
                    case TYPE_PARCELABLE_ARRAY_LIST:
                        if (!Utils.safeEquals(mBundle.getParcelableArrayList(param),
                                oParams.mBundle.getParcelableArrayList(param))) {
                            return false;
                        }
                        break;
                    default:
